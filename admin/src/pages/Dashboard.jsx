  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch recent orders - assuming an endpoint exists or we mock it for now since we didn't check
        // If specific endpoint for recent orders doesn't exist, we might fail here. 
        // Safer to mock the chart data and recent orders if we aren't sure of the API.
        // But the plan said "requires fetching orders". 
        // Let's try to fetch orders with a limit if the endpoint follows REST conventions.
        // Assuming /orders endpoint exists for admin. 
        // Based on previous exploration, we only saw dashboard endpoint usage.
        // Let's stick to the plan but be safe. If fetch fails, we show empty.
